...
% ─────────────────────────────────────────────────────────────────────────────
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Development Phases and Collaboration}
% ─────────────────────────────────────────────────────────────────────────────
...
Score calculation and ranking are performed locally in JavaScript after the final question, without a network request. The interface implements a dedicated loading animation system (\texttt{loadingOverlay} and \texttt{progressLoadingBar}) within \texttt{app.js} to handle short loading transitions when moving between screens and generating department information. This simulated processing improves perceived performance and user experience. Verified load-time, result-time, and concurrent-user measurements are not available and remain placeholders.
...
\textbf{Implementation note:} The table above describes the intended access-control design. In the current repository, the dashboard is a functional prototype built with static mock data in \texttt{src/js/dashboard.js}, displaying simulated total assessments, department distributions, and student records via \texttt{public/dashboard.html}. While the data layer and UI structure are established, backend authentication, roles, audit logging, and protected content-management operations are not yet implemented for production.
...
% ─────────────────────────────────────────────────────────────────────────────
During the 6-week industrial practice at the Haramaya University ICT Center, our team achieved several key learning outcomes:
\begin{itemize}
    \item \textbf{Full-Stack Technical Proficiency}: We successfully transitioned from theoretical classroom knowledge to practical application, mastering modern frameworks including React, Node.js, Express, and MongoDB.
    \item \textbf{Agile Project Management}: Participating in daily standups, sprint planning, and GitHub version control gave us firsthand experience in collaborative software engineering.
    \item \textbf{Requirements Engineering}: Engaging directly with students (via surveys) and Department Heads taught us how to translate abstract user needs into concrete software requirements.
    \item \textbf{AI Integration}: We learned how to integrate large language models (LLMs) into production applications, dealing with timeouts, prompts, and API rate limits.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Data Collection Bottlenecks}: Coordinating schedules with six different department heads for interviews was challenging and required persistence.
    \item \textbf{Scoring Algorithm Design}: Converting qualitative department preferences into a quantitative, normalized scoring algorithm required multiple iterations and HOD validation.
    \item \textbf{Time Constraints}: The 6-week timeline was extremely aggressive for a full-stack application, forcing us to strictly prioritize MVP features over "nice-to-have" additions.
\end{itemize}

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Recommendations}
% ─────────────────────────────────────────────────────────────────────────────
Based on our experience developing the CCI Department Choice Guidance System, we propose the following recommendations:

\subsubsection*{Recommendations for the Host Organization (ICT Center / CCI)}
\begin{itemize}
    \item \textbf{Future Student Placements}: We strongly recommend the ICT Center for future student placements. The supportive environment, access to cloud resources, and mentorship provided an excellent learning ground.
    \item \textbf{System Adoption}: We recommend that the CCI Dean's office officially adopts the developed system into the freshman orientation program for the upcoming academic year.
    \item \textbf{Continuous Content Updates}: A designated administrator should be assigned to update the department criteria annually to ensure the recommendation engine remains accurate.
\end{itemize}

\subsubsection*{Recommendations for Future Internship Programs}
\begin{itemize}
    \item \textbf{Pre-Internship Training}: A short boot-camp on version control (Git) and Agile methodologies before the internship begins would allow students to hit the ground running faster.
    \item \textbf{Extended Timeline}: Extending the development phase from 6 to 8 weeks would allow for deeper usability testing and final code refactoring before handover.
\end{itemize}
